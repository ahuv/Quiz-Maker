...
public class TestClass
{
			...
			throws FileNotFoundException, IOException, InputMismatchException, EmptyLineException
	{
		...
		case 2: // user is a teacher
			/*
			 * do {// checks teacher identification try { found = true; name =
			 * JOptionPane.showInputDialog("Please enter your username: "); String password =
			 * JOptionPane.showInputDialog("Please enter your password: "); users.checkTeacherPassword(name, password);
			 * // checkIdentification(users,sc,name); } catch (IncorrectPasswordException | IncorrectUsernameException
			 * e) { System.out.println(e); found = false; } }while (found == false);
			 */
			...
			while (true)
			{
				...
				case 3: // displays student results
					...
					{
						System.out.println(marks.getNames().get(i) + ": " + marks.getQuizzes().get(i) + " "
								+ marks.getGrades().get(i));
					}
					break;
				case 4: // deletes quiz
					deleteQuiz();
	...
	private static void deleteQuiz() throws IOException
	{
		Scanner scan = new Scanner(System.in);
		...
		try
		{
			...
			if (quizzes.deleteQuiz(scan.nextLine()))
			{
	...
	/**
	 * Initialization of the quiz program. Asks the user to choose his user status (teacher or student).
	 * 
	...
	/**
	 * Guides a teacher in the creation of a new Quiz object. The quiz is added to a list of quizzes available to
	 * students.
	 * 
	 * @param myQuiz
	 *            A NewQuiz object
	...
	private static void validateAnswer(char answer)
	{
		...
		while (answer != val1 && answer != val2 && answer != val3 && answer != val4 && answer != 'A' && answer != 'B'
				&& answer != 'C' && answer != 'D')
		{
			System.out.print("That is not a valid answer.");
			System.out.println(" Please re-enter your answer: ");
